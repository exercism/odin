...
The Odin language is a statically typed, imperative language that covers the same domain as C but with a package system, stricter type checking and sophisticated memory management.

In this exercise, we will introduce the minimum you need to write a program in Odin: comments, packages, variables, constants, and procedures.
