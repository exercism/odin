...
test_divisible_by_100_but_not_by_3_is_still_not_a_leap_year :: proc(
	...
	is_leap, _ := is_leap_year(1900)
	testing.expect(t, !is_leap)
}

@(test)
test_divisible_by_400 :: proc(t: ^testing.T) {
	is_leap, _ := is_leap_year(2000)
	testing.expect(t, is_leap)
}
