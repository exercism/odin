...
//
// If we order the variables strictly by Colors, Nationalities, Pets, Activities, and Drinks
// it takes 318 steps to solve the problem.
// If we re-arrange where variables that appears in the same rule are adjacent, it only takes
// 79 steps.
Variable :: enum u8 {
	Milk,
	Norwegian,
