# Track Developers - Frequently Asked Questions

This document captures decisions and rationales about how the Odin Exercism track exercises and documentation should be written, it complements the [Contributing guide][odin-contributing] (that you should read first).

If your question is not covered in the contributing guide or any of the FAQ documents below, or if after reading the documentation you still have questions, please see if there is already an open [issue][odin-issues] or post a question to the [Exercism forum](https://forum.exercism.org/c/support/8).

## Odin Exercism Github Repo
- [Why only one exercise per Pull Request?][why-only-one-exercise-per-pull-request]

## Contributing Exercises
- [How to Create a New Exercise][how-to-create-a-new-exercise]
- [How to Write an Exercise Solution Stub][how-to-write-an-exercise-solution-stub]
- [When Writing Tests, How To Compare Result vs. Expected for Slices?][when-writing-tests-how-to-compare-result-and-expected-for-slices]
- [How to Integrate Benchmarks in Odin Practice Exercises?][how-to-integrate-benchmarks-in-odin-practice-exercise]

## Tooling
- [How to Setup Your Machine to Contribute to the Track?][how-to-setup-your-machine-to-contribute-to-the-track]
- [The Odin Track Tools][odin-track-tools]

## Documentation
- [How To Develop Documentation for a Concept?][how-to-develop-documentation-for-a-concept]


[odin-contributing]: https://github.com/exercism/odin/blob/main/CONTRIBUTING.md
[odin-issues]: https://github.com/exercism/odin/issues

[why-only-one-exercise-per-pull-request]: https://github.com/exercism/odin/blob/main/dev/docs/faqs/why_only_one_exercise_per_pull_request.md
[when-writing-tests-how-to-compare-result-and-expected-for-slices]: https://github.com/exercism/odin/blob/main/dev/docs/faqs/when_writing_tests_how_to_compare_result_and_expected_for_slices.md
[how-to-integrate-benchmarks-in-odin-practice-exercise]: https://github.com/exercism/odin/blob/main/dev/docs/faqs/how_to_integrate_benchmarks_in_odin_practice_exercises.md
[how-to-develop-documentation-for-a-concept]: https://github.com/exercism/odin/blob/main/dev/docs/faqs/how_to_develop_documentation_for_a_concept.md
[how-to-setup-your-machine-to-contribute-to-the-track]: https://github.com/exercism/odin/blob/main/dev/docs/faqs/how_to_setup_your_machine_to_contribute_to_the_track.md
[odin-track-tools]: https://github.com/exercism/odin/blob/main/dev/docs/faqs/the_odin_track_tools.md
[how-to-create-a-new-exercise]: https://github.com/exercism/odin/blob/main/dev/docs/faqs/how_to_create_a_new_exercise.md
[how-to-write-an-exercise-solution-stub]: https://github.com/exercism/odin/blob/main/dev/docs/faq/how_to_write_an_exercise_solution_stub.md
