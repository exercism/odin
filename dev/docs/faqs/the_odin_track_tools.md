...
- `bin/configlet` is the exercism tool, the options include:
  ...
  - `sync`: if `configlet lint` reports inconsistencies with the [Problem Specification repository][exercism-problem-specification-repository], use `bin/configlet sync -u` to fix them.
